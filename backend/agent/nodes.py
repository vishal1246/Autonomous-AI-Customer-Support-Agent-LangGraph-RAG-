...
def classify_intent(
    ...
    Routing logic:
      billing intent  OR critical urgency  → search_documentation (then draft → human_review)
...
Provide the topic (short noun phrase) and a one-sentence summary."""
    ...
    # Always draft first so human reviewers have something to approve/edit.
    # draft_response node routes to human_review for billing/high/critical/complex.
    if classification["intent"] == "bug":
        goto = "bug_tracking"
    elif classification["intent"] in ["question", "feature"]:
        goto = "search_documentation"
    elif classification["intent"] == "billing":
        # Billing queries benefit from KB context (pricing, tax, refund policies).
        goto = "search_documentation"
    else:
        # complex, or any other intent
        goto = "draft_response"
...
"""
    ...
    needs_review = (
        ...
        or classification.get("intent") in ["complex", "billing"]
    )
